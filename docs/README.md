...
## Table of Contents
   ...
   * [Whimsy](#whimsy)
      ...
      * [Overview](#overview)
         ...
         * [Definition of Common Terms](#definition-of-common-terms)
            * [Test Suite](#test-suite)
            * [Test Case](#test-case)
            * [Fixture](#fixture)
            * [Tags](#tags)
            * [Fail Fast](#fail-fast)
               * [The --fail-fast](#the---fail-fast)
               * [The fail_fast TestSute](#the-fail_fast-testsute)
               * [The fail_fast TestList](#the-fail_fast-testlist)
         * [Typical Runloop](#typical-runloop)
            * [Test Collection/Discovery Step](#test-collectiondiscovery-step)
               * [Digression on collection implementation.](#digression-on-collection-implementation)
            * [Test Running Step](#test-running-step)
      * [Test Writing By Example](#test-writing-by-example)
         * [Writing A Verifier Test](#writing-a-verifier-test)
         * [Running Your Test](#running-your-test)
      * [Writing Your Own Test](#writing-your-own-test)
         * [Writing Your Own Fixtures](#writing-your-own-fixtures)
      * [Credits](#credits)
...
A `TestSuite` is a completely self-contained unit of testing which must contain
one or more `TestCase` instances. Test suites can rely on [Fixtures](#fixture),
have [tags](#tags) (which contained test cases will be tagged with), and be
marked [fail_fast](#fail-fast).

#### Test Case

A `TestCase` is not a unit of test that is not necessarily self-contained. An
example of not being self contained would be a test which parses the output of
a gem5 run for a specific config checking that its output matches a known gold
standard. Since this test case relies on gem5 running first, it would no longer
pass if ran on its own and is therfore not self-contained.

Test cases can also have all the metadata that a test suite has (Tags and Fixtures). However they cannot be marked [fail_fast](#fail-fast)

#### Fixture

Fixtures are items which possibly require setup and/or tearing down after
a [TestCase](#test-case) or a [TestSuite](#test-suite) has run. When tests are
run, they will be handed fixtures from their containing TestSuite, and will set
up any fixtures that are not already built. This allows test cases to
incrementally test results of gem5 runs or perform other incremental testing.
...
(X86, ARM, etc.), testing length (quick, long, etc.), and system emulation type
(se,fs). Indexes of tags are built by the `TestLoader` to query and run tests
by specifying command line arguments.

#### Fail Fast

Fail Fast (often written `fail_fast` here) has slightly different semantics
depending on the use case. In general it means that given a failure quickly
...
library will teardown fixtures, finish writing results, and exit.

##### The fail_fast TestSute

While executing a `TestSuite` that is marked `fail_fast`, if a `TestCase` in
that suite fails then the remaining tests in that TestSuite will be skipped. If
there are any remaining TestSuites to run, they will continue to run.

##### The fail_fast TestList

Inside of a `TestSuite`, test cases are stored in hierarchical lists classes
named `TestList`. In addition to some utility functions `TestList` instances
have a `fail_fast` attribute. When a test fails in a TestList the remaining
Test cases in that TestList will be skipped. If there are any remaining test
lists or cases outside of the failed one, inside of the currently executing
TestSuite, they will still be executed.
...
_Table of contents generated by [github-markdown-toc](https://github.com/ekalinin/github-markdown-toc)_
